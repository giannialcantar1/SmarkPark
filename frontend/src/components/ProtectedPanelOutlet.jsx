...
export default function ProtectedPanelOutlet({ allowedRoles = [] }) {
  ...
  if (!hasAnyRole(user?.role || user?.user_metadata?.role, allowedRoles)) {
    return <Navigate to="/no-access" replace state={{ from: location, deniedPath: location.pathname, registerAlert: true }} />
  }
