...
def normalize_user(row: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_status = row.get("approval_status") or row.get("estado_aprobacion") or row.get("status") or row.get("estado")
    status = str(raw_status).strip().lower() if raw_status not in (None, "") else None
    return {
        **row,
